...
import (
	...
	"pthash-go/pkg/pthash" // Import the public PHF package
	"runtime"
	"testing"
	"time"
)
...
func check[K comparable, F interface {
	...
	if f.IsMinimal() {
		...
		expectedSum := (n * (n - 1)) / 2 // Assumes n fits so n*n doesn't overflow uint64
		...
		for _, key := range keys {
			...
			sum += p // Potential overflow if n is very large (~ > 2^32)
		}
		// Only check sum if no overflow is likely
		if n <= (1 << 32) { // Heuristic check
		    if sum != expectedSum {
				return fmt.Errorf("check failed (minimal): sum mismatch (got %d, expected %d)", sum, expectedSum)
			}
		} else {
		     fmt.Println("Warning: Skipping sum check for minimal PHF due to large n (>2^32)")
		}
...
func TestInternalSinglePHFBuildAndCheck(t *testing.T) {
	...
	type E = core.RiceEncoder // Use the encoder implemented in Phase 2

	seed := uint64(time.Now().UnixNano())
	// Use smaller number of keys for faster testing
	//numKeysList := []uint64{10, 100, 1000, 10000, 50000}
					...
					for _, searchType := range searchTypes {
						for _, minimal := range []bool{true, false} {
							...
							t.Run(testName, func(t *testing.T) {
								...
								var bucketer B // = core.SkewBucketer{}

								builder := NewInternalMemoryBuilderSinglePHF[K, H, B](hasher, bucketer)
								...
								if err != nil {
									...
									// Optional: Dump state or retry with verbose=true?
								}
